...
def test_ingest_scores_mixed_dates_rejected(client: TestClient):
    _setup_players(client)
    response = client.post("/ingest/scores", json={"files": [
        "Date,GameNo,A,B,PtsAB,X,Y,PtsXY\n"
        "08-04-2024,1,Bhavin,Chets,21,Chan,Jayesh,9\n"
        "15-04-2024,2,Bhavin,Chan,21,Chets,Jayesh,9\n"  # different date
    ]})
    assert response.status_code == 422
    errors = response.json()["detail"]
    assert any("same date" in e.lower() for e in errors)
